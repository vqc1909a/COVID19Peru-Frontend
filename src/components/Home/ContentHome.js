...
const ContentHome = ({result, provincia}) => {
  ...
  return (
    <ContentHomeContainer
      ...
      style={window.location.pathname !== '/' & window.innerWidth < 992 ? {paddingBottom: "10vh"}:{}}
    >
      <DarkMode></DarkMode>
      {window.location.pathname.search(/departamentos/ig) !== -1 
      ?
        <Return></Return>
      :
        ''
      ...
      {window.location.pathname !== "/" & window.innerWidth <= 768
      ?
      <div className="wrapper-input1"><InputDropdownDepartamento darkMode={darkMode} setLoading={setLoadingDataProvincia}></InputDropdownDepartamento></div>
      :
      ''
      }
      <h1 className="text-big">
        ...
        <br></br> {window.location.pathname === '/' ? (Object.keys(result).length !== 0  ? result.name.toUpperCase() : "...Cargando") : (Object.keys(departamento).length !== 0 ? departamento.name.toUpperCase() : "...Cargando")}
      </h1>
      {window.location.pathname !== "/" && window.innerWidth > 768
      ?
        <div className="wrapper-input2"><InputDropdownDepartamento  darkMode={darkMode} setLoading={setLoadingDataProvincia}></InputDropdownDepartamento></div>
      :
      ''
      }
      ...
      <Fragment>
        ...
        <p
          className={`text-xl ${
            darkMode ? 'special-color-dark' : 'special-color'
          }`}
        >
          {Object.keys(result).length !== 0 ? numberWithComas(result.positivos) : "...Cargando"}
        </p>
        ...
        <div className="mujeres flex">
          ...
          >
            {Object.keys(result).length !== 0 ? numberWithComas(result.mujeres_infectados) : "...Cargando"}
          </p>
        ...
        <div className="varones flex">
          ...
          >
            {Object.keys(result).length !== 0 ? numberWithComas(result.hombres_infectados) : "...Cargando"}
          </p>
        ...
        <div className="fallecidos flex">
          ...
          >
            {Object.keys(result).length !== 0 ? numberWithComas(result.fallecidos) : "...Cargando"}
          </p>
          <span className={`text-normal`}>Fallecidos</span>
        </div>
        {window.location.pathname === "/"
        ?
        <div className="recuperados flex">
          <svg
            width="33"
            height="31"
            viewBox="0 0 33 31"
          ...
          >
            <path
              ...
              d="M26.954 12.9467C23.6433 12.9467 20.954 15.6334 20.954 18.9467C20.954 22.2601 23.6433 24.9467 26.954 24.9467C30.2646 24.9467 32.954 22.2601 32.954 18.9467C32.954 15.6334 30.2646 12.9467 26.954 12.9467ZM30.2873 19.6134H27.6206V22.2801H26.2873V19.6134H23.6206V18.2801H26.2873V15.6134H27.6206V18.2801H30.2873V19.6134ZM21.5846 25.7374C20.1073 27.1854 18.5513 28.6961 16.954 30.2801C8.38065 21.7721 0.953979 15.4174 0.953979 9.20273C0.953979 0.177397 12.7566 -1.92527 16.954 5.2774C21.1206 -1.87194 32.954 0.14673 32.954 9.20273C32.954 10.1974 32.746 11.1974 32.39 12.2067C30.902 11.0041 29.0113 10.2801 26.954 10.2801C22.1753 10.2801 18.2873 14.1681 18.2873 18.9467C18.2873 21.6974 19.5806 24.1481 21.5846 25.7374Z"
              fill="#100E0E"
            />
          </svg>
          <p
          ...
          >
             {Object.keys(result).length !== 0 ? numberWithComas(result.recuperados) : "...Cargando"}
          </p>
          <span className={`text-normal`}>Recuperados</span>
        </div>
        :
        ""
        }
      </div>
      <button onClick={() => scrollDown()} type="button" className={darkMode ? 'button-dark' : ''}>
        Más detalles
      </button>
      </Fragment>
      :
